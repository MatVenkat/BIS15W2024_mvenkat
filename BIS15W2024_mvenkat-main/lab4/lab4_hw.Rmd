...
```{r}
taxa <- c("taxon", "common name", "class", "order", "family", "genus")
df <- data.frame(taxa)
```
...
mean(mean1, na.rm=T)
#carnivore have larger mean.hra.m2
```
...
```{r}
animaldata <- select(homerange, mean.mass.g, log10.mass, family, genus)

owls <- filter(animaldata, family=="")
owls_new <- rename(owls, mean_mass_g="mean.mass.g",
                   log10mass= "log10.mass",
                   family="family",genus="genus", species= "species")
max(deer$mean.mass.g)
filter(owls, mean.mass.g >=4000)
```
...
birds_new - select(birds, genus, species, mean.mass.g, mean.hra.2, log10.mass, log10.hra)
arrange(birds_new, mean.hra.m2)
```
