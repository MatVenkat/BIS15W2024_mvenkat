...
msleep24 <- msleep %&%
  ...
  select(name, core, sleep_total_24, sleep_total) %&%
...
```{r}
msleep %>%
  summarize(n_genera=n_distinct(genus), n_order) #this is going to count the number of genera in msleep
...
msleep%>%
  ...
  select(orer, genus, mean_brain_wt)
```
